...
def run_ga(individuos, n_clientes, n_centroslocales, n_centrosregionales, n_periodos, n_productos, n_vehiculos_s, n_vehiculos_p, capacidad_cr, capacidad_cl, capacidad_vehiculos_p, capacidad_vehiculos_s, demanda_clientes, demand_cl_poblation, inventario, costo_instalaciones_cl, costo_instalaciones_cr, costo_compraproductos, costo_transporte, costo_inventario, costo_rutas_s, costo_rutas_p, costo_vehiculos_s, costo_vehiculos_p, costo_humano, w1, w2, final_inventarioQ, final_inventarioI, valores_f1, valores_f2, valores_ft, demand_cr_poblation, n_generaciones, prob_mut, colector, idx_pa, timer):
    ...
    for _ in tqdm(range(n_generaciones)):
        ...
        colect_temp = []
        fit_dict = {}
        for id_f, fit in enumerate(fitness_poblation):
            fit_dict[id_f] = fit
        fit_ord = sorted(fit_dict.items(), key=operator.itemgetter(1), reverse=False)
        c_best = int(0.07*len(individuos))
        c_worst = round(0.03*len(individuos))
        for c_b in range(c_best):
            colect_temp.append(fit_ord[c_b])
        for c_w in range(c_worst):
            colect_temp.append(fit_ord.pop())
        idx_temp = []
        ind_temp = []
        cr_dem_temp = []
        cl_dem_temp = []
        ...
        f2_temp = []
        fit_temp = []
        for f_orf in colect_temp:
            idx_temp.append(f_orf[0])
            ind_temp.append(individuos[f_orf[0]])
            cr_dem_temp.append(demand_cr_poblation[f_orf[0]])
            cl_dem_temp.append(demand_cl_poblation[f_orf[0]])
            q_temp.append(Q_poblation[f_orf[0]])
            i_temp.append(I_poblation[f_orf[0]])
            f1_temp.append(f1_poblation[f_orf[0]])
            f2_temp.append(f2_poblation[f_orf[0]])
            fit_temp.append(f_orf[1])
        colector.value[idx_pa] = [ind_temp, cr_dem_temp, cl_dem_temp, q_temp, i_temp, f1_temp, f2_temp, fit_temp]
        time.sleep(timer)
        # while (len(colector.value[1][0]) < int(0.1*len(individuos))) or (len(colector.value[0][0]) < int(0.1*len(individuos))):
        #        print("waiting...")
        if idx_pa == 0:
        ...
        # reemplazo
        replaced = []
        for id_c in range(int(0.1*len(individuos))):
            id_r = np.random.randint(len(individuos))
            while id_r in replaced:
                id_r = np.random.randint(len(individuos))
            individuos[id_r] = colector.value[idx_pa][0][id_c]
            demand_cr_poblation[id_r] = colector.value[idx_pa][1][id_c]
            demand_cl_poblation[id_r] = colector.value[idx_pa][2][id_c]
            Q_poblation[id_r] = colector.value[idx_pa][3][id_c]
            I_poblation[id_r] = colector.value[idx_pa][4][id_c]
            f1_poblation[id_r] = colector.value[idx_pa][5][id_c]
            f2_poblation[id_r] = colector.value[idx_pa][6][id_c]
            fitness_poblation[id_r] = colector.value[idx_pa][7][id_c]
            replaced.append(id_r)

        # almacenaje del mejor
        colector.value[2].append([colector.value[idx_pa][0][0], colector.value[idx_pa][1][0], colector.value[idx_pa][2][0], colector.value[idx_pa][3][0], colector.value[idx_pa][4][0], colector.value[idx_pa][5][0], colector.value[idx_pa][6][0], colector.value[idx_pa][7][0]])

    return individuos, demand_cr_poblation, demand_cl_poblation, Q_poblation, I_poblation, f1_poblation, f2_poblation, fitness_poblation
