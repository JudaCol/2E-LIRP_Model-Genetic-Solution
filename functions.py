...
def read_data(n_clientes, n_productos, n_periodos, n_vehiculos_p, n_vehiculos_s, n_centrosregionales, n_centroslocales):
    ...
    costo_rutas_s = [[hoja_costo_rutas_s.cell(row=i, column=j).value for j in range(3, 3+n_centroslocales+n_clientes)] for i in range(2, 2+n_centroslocales+n_clientes)]
        ...
        for l in range(n_productos):
            ...
            for w in range(n_periodos):
                ...
                if w == 0:
                    ...
                    valoresI[centroshabs[u] + 1].append(It)
                    iper1 = It
                    # revisar si este periodo causa problemas con las condiciones del algoritmo de inventarios
                    carga = carga - Q
                elif w == n_periodos - 1:
                    Q = carga
                    valoresQ[centroshabs[u] + 1].append(Q)
    ...
    for cr in cr_habs:
        ...
        for pr in range(n_productos):
            for t in range(n_periodos):
                costprod += matrizQp[pr][t] * costo_compraproductos[pr, t]
                costtrans += matrizQp[pr][t] * costo_transporte[cr - 1, (n_periodos * t) + pr]
                costinv += matrizIp[pr][t] * costo_inventario[cr - 1, pr]
...
# documentar inventarios y lineas adicionales
